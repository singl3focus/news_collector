...
def is_good_news(post, change_stock=0.01):
    ...
    if count_up_change_2 > count_down_change_2 or (count_up_change > count_down_change and tonality["label"] == "POSITIVE"):
        return True, Post(text=post.text, tonality=1, channel_id=post.channel_id, channel_title=post.channel_title, timestamp=post.timestamp)
